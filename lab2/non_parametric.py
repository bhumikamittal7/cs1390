...
class DTN:
    ...
    def classify(self, x):
        if self.fi == None:             # leaf
            ...
            return self.lc.classify(x)          #lc means left child
        else:
            return self.rc.classify(x)          #rc means right child
...
def auto_xval(data, labels, k=10, decision_tree=True, verbose=True, config=None):
    ...
    #k fold means split the data into k parts and use each part as test data once (train on k-1 and test on k)
    fold_size = int(len(data)/k)    #divide the data into k parts
    fold_indices = [(i*fold_size, (i+1)*fold_size) for i in range(k)] #for each part, get the start and end index
    folds = [(data[start:end], labels[start:end]) for (start, end) in fold_indices] #get the data and labels for each part
    data_folds = [fold[0] for fold in folds]    #get the data for each part
    labels_folds = [fold[1] for fold in folds]  #get labels for each part
    #np.split(data, k) #split the data into k parts
    #np.split(labels, k) #split the labels into k parts

    score_sum = 0
    for i in range(k):
        ##TODO USE ith fold for testing, remaiing for traininng
        data_test = data_folds[i]
        labels_test = labels_folds[i]
        data_train = np.vstack([data_folds[j] for j in range(k) if j != i])
        labels_train = np.vstack([labels_folds[j] for j in range(k) if j != i])
